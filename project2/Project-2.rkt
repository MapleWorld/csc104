...
(define (invisible-width the-width)
  (rectangle the-width 0 "outline" "transparent"))
...
; ★ Define imagine:
; Completed!!! Not yet, some problem with check expect doesn't match
(define (imagine a-text-or-number)
  (align-beside "baseline"
                (invisible-width text-size)
                (text->image (textify a-text-or-number) text-size text-color)
                (invisible-width text-size)))
...
(define (column table i)
  (if [(>= i 1) (local [(define remainingList (map rest table))
                        (define shorterColumns (column remainingList (- i 1)))]
                  shorterColumns)]
      [else (map first table)]))
...
(define (columns table)
  (if [(= (length (first table)) 1) (map first table)]
      [else (local [(define columnList (map first table))
                    (define remainingList (map rest table))
                    (define rowsIntoColumns (columns remainingList))]
              (list columnList rowsIntoColumns))]))
